...
1. Configuration binds one protocol ID to one network, INIT transaction, deployment heights, spec
   ...
   fails.
2. Bitcoin Core RPC supplies authoritative block order. ZMQ endpoints are optional and are subscribed to
...
4. MySQL stores authoritative blocks, transactions, events, objects, carrier states, chapters,
   checkpoints, conflicts, and reorg journals. Mempool records stay in their own table and never
   contribute to authoritative state, counters, or roots.
...
7. The verified explorer gateway signs pipeline A's tuple, obtains pipeline B's independently signed
   ...
   authoritative height. It repeats the verification after each data read and withholds the response if
...
- a block ingestion loop, and the writes that persist authoritative blocks, transactions, and events
